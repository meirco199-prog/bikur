...
export const KEY_NAMES = ['TWELVEDATA_KEY', 'TIINGO_KEY', 'MARKETSTACK_KEY', 'FINNHUB_KEY', 'FMP_KEY', 'ALPHAVANTAGE_KEY', 'FRED_KEY', 'EODHD_KEY', 'TELEGRAM_BOT_TOKEN', 'TELEGRAM_CHAT_ID', 'RESEND_KEY', 'ALERT_EMAIL', 'ANTHROPIC_API_KEY', 'EDGAR_UA', 'CRON_BATCH', 'STOOQ_ENABLED', 'COUNCIL_SECRET', 'GH_COUNCIL_TOKEN'];
export const KEY_LABELS = { COUNCIL_SECRET: 'AI Council — סוד משותף לערוץ הכתיבה של ChatGPT (POST /council/comment)', GH_COUNCIL_TOKEN: 'AI Council — GitHub PAT מצומצם (bikur בלבד, Issues: write) לפרסום תגובות ב-Issue #25', TWELVEDATA_KEY: 'Twelve Data (מחירים יומיים + quote) — חינם 800/יום', TIINGO_KEY: 'Tiingo (מחירים יומיים) — חינם', MARKETSTACK_KEY: 'Marketstack (בורסת תל אביב) — חינם 100/חודש', FINNHUB_KEY: 'Finnhub (quote, אנליסטים, חדשות) — חינם', FRED_KEY: 'FRED (מאקרו, VIX, ריבית) — חינם', ALPHAVANTAGE_KEY: 'Alpha Vantage (סנטימנט, ETF, insider) — חינם 25/יום', FMP_KEY: 'FMP (screener, תחזיות, יעדי מחיר) — חינם 250/יום', EODHD_KEY: 'EODHD (בורסת ת"א) — בתשלום', TELEGRAM_BOT_TOKEN: 'Telegram bot token', TELEGRAM_CHAT_ID: 'Telegram chat id', RESEND_KEY: 'Resend (אימייל) API key', ALERT_EMAIL: 'כתובת אימייל להתראות', ANTHROPIC_API_KEY: 'Anthropic API key (עוזר AI; אחרת Workers AI)', EDGAR_UA: 'User-Agent ל-SEC (שם + אימייל)', CRON_BATCH: 'נכסים לכל הפעלת cron', STOOQ_ENABLED: 'Stooq (1 = מופעל; חסום לרוב מ-Cloudflare)' };
