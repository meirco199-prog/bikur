...
export function evaluateRule(rule, snap, prev, analysis){
  ...
  switch (rule.type){
    ...
    case 'unusual_volume': { const rv = t?.relVol ?? snap.relVol; return isNum(rv) && rv >= p.relVol ? mk(`מחזור חריג ב-${snap.symbol}: x${round(rv, 1)} מהממוצע`, 'מחזור חריג מלווה לרוב אירוע (דוח/חדשות/מוסדיים)') : null; }
    case '_unused_volume': return t && isNum(t.relVol) && t.relVol >= p.relVol ? mk(`מחזור חריג ב-${snap.symbol}: x${round(t.relVol, 1)} מהממוצע`, 'מחזור חריג מלווה לרוב אירוע (דוח/חדשות/מוסדיים)') : null;
    ...
    case 'analyst_change': { const d = analysis?.analyst?.trendDelta ?? snap.analystTrendDelta; return isNum(d) && d !== 0 && prev ? mk(`שינוי קונצנזוס ב-${snap.symbol}: ${d > 0 ? '+' : ''}${d} דירוגי Buy`, 'דעת אנליסטים היא אות אחד — לא אמת; שינוי כיוון חשוב יותר מהרמה') : null; }
    case 'material_news': { const n = (analysis?.sentiment?.clusters || snap.materialNews || []).find((c) => (c.material ?? true) && c.publishedAt && daysBetween(c.publishedAt.slice(0, 10), snap.date) <= 1); return n && prev?.lastNews?.title !== n.title ? mk(`חדשות מהותיות ב-${snap.symbol}: ${n.title} (${n.publisher}, ${n.publishedAt.slice(0, 10)}) ${n.url}`, `אירועים: ${n.events.join(', ')}; סנטימנט ${n.sentiment.label}`) : null; }
