...
test('POST /universe/refresh דורש אימות/סוד; בלי FMP_KEY: ויקיפדיה + דגימה מרובדת, הוספה בקצב מוגבל', async () => {
  ...
  assert.equal(r.status, 200); assert.equal(j.universe.ok, true, JSON.stringify(j.universe)); assert.equal(j.universe.source, 'wikipedia'); assert.equal(j.universe.sizeKnown, false, 'בלי FMP אין נתוני גודל'); assert.equal(j.universe.selected, 504, 'כל חברי המדד'); assert.ok(j.universe.added >= 498, 'כולם נוספים (חוץ מאלה שכבר ביקום הבסיס)'); assert.equal(j.universe.pending, 0); assert.equal(j.calendar.ok, false);
});
...
test('יקום מכני: כל חברי המדד נכנסים; maxAdd מגביל כשמבקשים; reset מסיר מכניות שלא מוחזקות; דגימה יציבה בין ריצות', async () => {
  const { installMockFetch } = await import('./mock-providers.mjs'); installMockFetch();
  ...
  const r1 = await refreshMechanicalUniverse(ctx, { cap: 100, maxAdd: 25 });
  ...
  const r2 = await refreshMechanicalUniverse(ctx, { cap: 100, maxAdd: 25 });
  ...
  const r3 = await refreshMechanicalUniverse(ctx, { reset: true, maxAdd: 10, cap: 100 });
