...
- **כבוי עד שבעל הריפו מגדיר `OPENAI_API_KEY` ב-Actions Secrets** — זו ההרשאה להוצאה. דגם: repo variable `OPENAI_MODEL`
  (ברירת מחדל `gpt-5`), מאמץ חשיבה: variable `OPENAI_REASONING` (ברירת מחדל `low`). מפתחות בטקסט מוסתרים לפני הפרסום.
  אומדן עלות: כ-25K טוקני קלט + עד 8K פלט (כולל טוקני חשיבה) לריצה.
