...
export async function render(main){
  ...
  ${card('התיק שלי (וירטואלי)', pos.length ? `<table><thead><tr><th>נכס</th><th class="num">כמות</th><th class="num">קניתי ב-</th><th class="num">עכשיו</th><th class="num">רווח/הפסד</th><th></th></tr></thead><tbody>${pos.map((p) => `<tr><td><a href="#/asset/${esc(p.symbol)}">${esc(nameOf(by.get(p.symbol) || { symbol: p.symbol }))}</a></td><td class="num">${p.qty}</td><td class="num">${fmt.num(p.price)}</td><td class="num">${fmt.num(p.current)}</td><td class="num ${cls(p.pnl)}">${fmt.usd(p.pnl)} (${fmt.pct(p.pnlPct, 1, true)})</td><td><button class="btn sm" data-sell="${esc(p.symbol)}">מכור</button></td></tr>`).join('')}</tbody></table><p class="muted">שווי ${fmt.usd(paper.marketValue)} · רווח לא ממומש <span class="${cls(paper.unrealized)}">${fmt.usd(paper.unrealized)}</span> · ממומש <span class="${cls(paper.realized)}">${fmt.usd(paper.realized)}</span></p>` : '<div class="empty">התיק הווירטואלי ריק. לחץ "קנייה (וירטואלית)" על נכס כדי לעקוב אחרי ביצועי המערכת בלי כסף אמיתי.</div>')}
  ...
  main.__simpleClick = async (e) => {
    const b = e.target.closest('[data-buy],[data-sell],[data-watch]'); if (!b) return;
