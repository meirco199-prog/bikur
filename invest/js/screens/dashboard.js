...
import { el, esc, fmt, isNum, cls } from '../core/util.js';
...
const symRow = (s) => `<tr class="clickable" onclick="location.hash='#/asset/${esc(s.symbol)}'"><td>${symLink(s.symbol, s.name)}</td><td class="num">${fmt.num(s.price)}</td><td>${pctCell(s.dailyChange)}</td><td>${scoreBar(s.score)}</td><td>${sigBadge(s.signal)}</td></tr>`;
...
export async function render(main){
  ...
  main.innerHTML = `
  <div class="page-head"><div><h1>תמונת מצב</h1><span class="muted">נכון ל-${esc(rank?.date || regime?.date || '—')} · ${noRank ? '<span class="tag stale">אין דירוג יומי עדיין — הרץ עיבוד בהגדרות</span>' : `${rank.analyzed} נכסים נותחו`}</span></div>
  ...
  <div class="grid g4" style="margin-bottom:1rem">
    ${card('MARKET STATUS', regime && regime.risk ? `<div class="stack"><div class="row"><span class="tag ${regime.risk === 'Risk On' ? 'fact' : regime.risk === 'Risk Off' ? 'missing' : ''}">${esc(regime.risk)}</span><span class="tag ${regime.trend === 'Bull Trend' ? 'fact' : regime.trend === 'Bear Trend' ? 'missing' : 'stale'}">${esc(regime.trend)}</span></div>
    ...
    ${card(`PORTFOLIO <span class="muted num">${fmt.ils(size)}</span>`, bal ? `<div class="stack"><div class="muted">תיק מאוזן מומלץ (MODEL) · ${bal.positions.length - 1} נכסים + מזומן</div>
    ...
    ${card('PAPER TRADING', paper && !paper.error ? `<div class="grid g2">${kpi('שווי פוזיציות', fmt.usd(paper.marketValue))}${kpi('P&L לא ממומש', fmt.usd(paper.unrealized), cls(paper.unrealized))}${kpi('P&L ממומש', fmt.usd(paper.realized), cls(paper.realized))}${kpi('אחוז הצלחה', fmt.pct(paper.winRate, 0))}</div><a href="#/paper" class="muted">תיק וירטואלי →</a>` : '<div class="empty">—</div>')}
    ${card('MARKET RISK', regime?.rules ? `<ul class="list" style="font-size:.85rem">${regime.rules.filter((r) => r.passed !== null && ((r.dir === 'riskoff' || r.dir === 'bear') ? r.passed : !r.passed)).slice(0, 6).map((r) => `<li class="neg">${esc(r.desc)}</li>`).join('') || '<li class="pos">אין דגלי סיכון פעילים בכללי המשטר</li>'}</ul>${regime.missingInputs?.length ? `<div class="muted">חסר: ${esc(regime.missingInputs.join(', '))}</div>` : ''}` : '<div class="empty">—</div>')}
  </div>
  <div class="grid g2" style="margin-bottom:1rem">
    ${card('TOP OPPORTUNITIES', symTable(pick(rank?.categories?.bestOverall).slice(0, 8)), '<a class="muted" href="#/opportunities">הכול →</a>')}
    ${card('WATCHLIST', watch?.items?.length ? symTable(watch.items.map((w) => w.snapshot || { symbol: w.symbol })) : '<div class="empty">רשימת המעקב ריקה — הוסף נכסים מדף הנכס</div>', '<a class="muted" href="#/watchlist">ניהול →</a>')}
    ${card('BUY SIGNALS', symTable(buy), `<span class="tag fact">${rank?.categories?.buySignals?.length || 0}</span>`)}
    ${card('SELL / REDUCE SIGNALS', symTable(sell), `<span class="tag missing">${rank?.categories?.sellSignals?.length || 0}</span>`)}
    ${card('RECENT IMPORTANT NEWS', news.length ? `<ul class="list" style="font-size:.86rem">${news.map((n) => `<li><a href="#/asset/${esc(n.symbol)}"><b>${esc(n.symbol)}</b></a> · <a href="${esc(n.url)}" target="_blank" rel="noopener">${esc(n.title)}</a> <span class="muted">${esc(n.publisher || '')} · ${fmt.date(n.publishedAt)}</span></li>`).join('')}</ul>` : '<div class="empty">אין חדשות עדיין</div>')}
    ${card('ALERTS', alerts?.length ? `<ul class="list" style="font-size:.86rem">${alerts.slice(0, 8).map((a) => `<li><b>${esc(a.symbol)}</b> ${esc(a.label || a.type)}: ${esc(a.what)} <span class="muted">${fmt.ago(a.ts)}</span></li>`).join('')}</ul>` : '<div class="empty">אין התראות ב-14 הימים האחרונים</div>', '<a class="muted" href="#/alerts">כללים →</a>')}
  </div>
